import React from 'react'
import pattern from "../../assets/cover.jpg"
import Playstore from "../../assets/play_store.png"
import Appstore from "../../assets/app_store.png"

const bannerImg= {
    backgroundImage: `url(${pattern})`,
    backgroundRepeat: 'no-repeat',
    backgroundSize: 'cover',
    backkgroundPosition: 'center',
    height: '100%',
    width: '100%',
}

function Banner() {
  return (
    <div className='container pb-14'>
    <div
    className='text-white py-10 sm:min-h-[400px] sm:grid sm:place-items-center rounded-xl'
    style={bannerImg}>
        <div>
            <div className='space-y-6 max-w-xl mx-auto'>
                <h1 data-aos="fade-up" className='text-2xl text-center sm:text-4xl font-semibold '>Get Started With our app</h1>
                <p data-aos="fade-up" className='text-center sm:px-20 '>Lorem ipsum dolor sit amet consectetur adipisicing elit. In commodi eveniet suscipit quam incidunt rem possimus! Voluptatum magnam ut amet!</p>
                <div data-aos="fade-up" className='flex flex-wrap items-center justify-center gap-4'>
                    <a href='#'>
                        <img src={Playstore}
                        className='max-w-[150px] sm:max-w-[120px] md:max-w-[200px]'/>
                    </a>
                    <a href='#'>
                        <img src={Appstore}
                        className='max-w-[150px] sm:max-w-[120px] md:max-w-[200px]'/>
                    </a>
                </div>
            </div>

        </div>
        </div>        
    </div>
  )
}

export default Banner